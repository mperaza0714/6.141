...
def msg_from_pose(pos, yaw = 0.0):
    '''
    Returns a ROS message of type geometry_msgs.msg.Pose from a 1x2 array [x, y] and the yaw angle.
    '''
    msg = Pose()
    msg.position.x = pos[0]
    msg.position.y = pos[1]
    # q = tf.transformations.quaternion_from_euler((q.x,q.y,q.z,q.w))
    msg.orientation.w = 1.0
    return msg

def dot(a, b):
    return np.sum(a*b, axis=1)
    # return sum(a[:,i]*b[:,i] for i in xrange(a.shape[1]))

def wrap(a, lo=0.0, hi=1.0):
    return np.where(a >= lo, np.where(a <= hi, a, hi), lo)
